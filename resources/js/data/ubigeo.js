/**
 * Peru UBIGEO helper data
 * Provides department, province, and district lists for form selects.
 *
 * getDepartamentos()           -> string[]
 * getProvincias(departamento)  -> string[]
 * getDistritos(depto, prov)    -> string[]
 */
...
export function getProvincias(departamento) {
    if (!departamento || !UBIGEO[departamento]) return [];
    return Object.keys(UBIGEO[departamento]).sort();
}

export function getDistritos(departamento, provincia) {
    if (!departamento || !provincia) return [];
    const provData = UBIGEO[departamento];
    if (!provData || !provData[provincia]) return [];
    return [...provData[provincia]].sort();
}
